...
This project also uses `llvm` tools, for code coverage. And `clang-format` for formatting. And (of course) `valgrind`.  


This will get you all the tools you need to build and test the project:  
```bash
sudo apt install llvm llvm-tools clang valgrind clang-format
```
