...
const char *test_stack_simple() {
  stack *s = stack_new(test_context, sizeof(const char *));
...
const char *test_stack_non_pointer() {
  stack *s = stack_new(test_context, sizeof(test_stack_obj));
...
const char *test_stack_allocation_failures() {
  ...
  TEST_ASSERT(stack_new(test_context, sizeof(const char *)) == NULL, "Initial allocation failures should result in null stack");
